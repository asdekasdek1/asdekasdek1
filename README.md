...
contract LevelSystem {
    ...
    function gainExperience(uint256 amount) external {
        ...
        uint256 newLevel = experience[msg.sender] / 100;
...
contract LevelUp {
    ...
    function gainXp(uint256 amount) external {
        ...
        uint256 newLevel = xp[msg.sender] / 50;
        level[msg.sender] = newLevel;
...
contract SparkCore {
    ...
    function spark() external {
        if (block.timestamp <= lastSpark[msg.sender] + 11 minutes) {
            sparks[msg.sender] += 1;
        } else {
            sparks[msg.sender] = 1;
        }
...
contract FluxCore {
    mapping(address => uint256) public fluxes;
    mapping(address => uint256) public lastFlux;

    event Fluxed(address indexed user, uint256 level);

    function flux() external {
        if (block.timestamp <= lastFlux[msg.sender] + 8 minutes) {
            fluxes[msg.sender] += 1;
        } else {
            fluxes[msg.sender] = 1;
        }
        lastFlux[msg.sender] = block.timestamp;
        emit Fluxed(msg.sender, fluxes[msg.sender]);
    }

    function getFluxes(address user) external view returns (uint256) {
        return fluxes[user];
    }
}
// SPDX-License-Identifier: MIT
...
contract BoltCore {
    ...
    function bolt() external {
        if (block.timestamp <= lastBolt[msg.sender] + 9 minutes) {
            bolts[msg.sender] += 1;
        } else {
            bolts[msg.sender] = 1;
        }
...
contract EmberCore {
    ...
    function light() external {
        if (block.timestamp <= lastEmber[msg.sender] + 5 minutes) {
            embers[msg.sender] += 1;
        } else {
            embers[msg.sender] = 1;
        }
...
contract AuraCore {
    ...
    function gain() external {
        if (block.timestamp <= lastAura[msg.sender] + 12 minutes) {
            auras[msg.sender] += 1;
        } else {
            auras[msg.sender] = 1;
        }
...
contract SparkLite {
    ...
    function spark() external {
        if (block.timestamp <= lastSpark[msg.sender] + 6 minutes) {
            sparks[msg.sender] += 1;
        } else {
            sparks[msg.sender] = 1;
        }
...
contract PulseLite {
    ...
    function pulse() external {
        if (block.timestamp <= lastPulse[msg.sender] + 9 minutes) {
            pulses[msg.sender] += 1;
        } else {
            pulses[msg.sender] = 1;
        }
...
contract GlowLite {
    ...
    function glow() external {
        if (block.timestamp <= lastGlow[msg.sender] + 11 minutes) {
            glows[msg.sender] += 1;
        } else {
            glows[msg.sender] = 1;
        }
...
contract BoltLite {
    ...
    function bolt() external {
        if (block.timestamp <= lastBolt[msg.sender] + 7 minutes) {
            bolts[msg.sender] += 1;
        } else {
            bolts[msg.sender] = 1;
        }
...
contract NovaLite {
    ...
    function activate() external {
        if (block.timestamp <= lastNova[msg.sender] + 10 minutes) {
            novas[msg.sender] += 1;
        } else {
            novas[msg.sender] = 1;
        }
